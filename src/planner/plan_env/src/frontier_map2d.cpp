...
void FrontierMap2D::clearFrontiers()
{
  // Clear all frontier lists
  size_t total_frontiers = frontiers_.size() + dormant_frontiers_.size() + candidate_frontiers_.size();
  frontiers_.clear();
  dormant_frontiers_.clear();
  candidate_frontiers_.clear();

  // Reset frontier flag buffer to NONE
  std::fill(frontier_flag_.begin(), frontier_flag_.end(), NONE);

  ROS_INFO("[FrontierMap2D] Cleared all frontiers for new episode (%zu total frontiers removed)", total_frontiers);
}
