...
void SDFMap2D::initMap(ros::NodeHandle& nh)
{
  ...
  md_->permanent_obstacle_buffer_ = vector<char>(mp_->buffer_size_, 0);
  ...
  // Initialize multi-source value map if enabled
  nh.param("value_map/use_multi_source", use_multi_source_value_map_, false);
  if (use_multi_source_value_map_) {
    multi_source_value_map_.reset(new MultiSourceValueMap(this, nh));
    ROS_INFO("[SDFMap2D] Multi-source value map enabled");
  } else {
    ROS_INFO("[SDFMap2D] Using single-source value map");
  }

  // Initialize IG value map if enabled
  nh.param("ig_value_map/enable", use_ig_value_map_, true);
  if (use_ig_value_map_) {
    ig_value_map_.reset(new IGValueMap(this, nh));
    ROS_INFO("[SDFMap2D] IG value map enabled");
  } else {
    ROS_INFO("[SDFMap2D] IG value map disabled");
  }
    ...
    const Eigen::Vector3d& camera_pos, vector<Eigen::Vector2i>& free_grids)
{
  ...
  while (!md_->cache_voxel_.empty()) {
    ...
    // Skip permanent obstacles - they should not be updated by depth sensor
    if (md_->permanent_obstacle_buffer_[adr] != 0) {
...
void SDFMap2D::setPermanentObstacle(const Eigen::Vector2d& pos)
{
  // Mark a grid cell as permanent obstacle that won't be cleared by depth updates
  // Used for stuck recovery to prevent agent from repeatedly trying blocked paths
  Eigen::Vector2i idx;
  posToIndex(pos, idx);
  if (!isInMap(idx)) return;

  int adr = toAddress(idx);
  md_->permanent_obstacle_buffer_[adr] = 1;
  md_->occupancy_buffer_[adr] = mp_->clamp_max_log_;
  ROS_DEBUG("[SDFMap2D] Marked permanent obstacle at (%.2f, %.2f)", pos.x(), pos.y());
}

bool SDFMap2D::isPermanentObstacle(const Eigen::Vector2i& idx)
{
  if (!isInMap(idx)) return false;
  return md_->permanent_obstacle_buffer_[toAddress(idx)] != 0;
}

void SDFMap2D::clearPermanentObstacles()
{
  // Clear all permanent obstacles (called on episode reset)
  std::fill(md_->permanent_obstacle_buffer_.begin(), md_->permanent_obstacle_buffer_.end(), 0);
  ROS_INFO("[SDFMap2D] Cleared all permanent obstacles");
}

void SDFMap2D::setCurrentNavTargetObjectId(int object_id)
{
  // Forward to MapROS for VLM frame collection
  if (map_ros_) {
    map_ros_->setCurrentNavTargetObjectId(object_id);
  }
}
...
void SDFMap2D::resetAllBuffers()
{
  ROS_INFO("[SDFMap2D] Resetting all map buffers for new episode");

  // Reset occupancy buffers
  ...
  // Clear occupancy update tracking
  md_->occupancy_need_clear_.clear();
  ...
  ROS_INFO("[SDFMap2D] Map reset complete");
}
