...
class SDFMap2D : public std::enable_shared_from_this<SDFMap2D> {
public:
  SDFMap2D() = default;
  ...
  // Permanent obstacle marking (won't be cleared by depth updates)
  void setPermanentObstacle(const Eigen::Vector2d& pos);
  bool isPermanentObstacle(const Eigen::Vector2i& idx);
  void clearPermanentObstacles();

  // VLM verification support - forward to MapROS
  void setCurrentNavTargetObjectId(int object_id);
  ...
  shared_ptr<MultiSourceValueMap> multi_source_value_map_;
  bool use_multi_source_value_map_;  ///< Flag to enable multi-source mode

  // Information Gain (IG) value map for exploration guidance
  shared_ptr<IGValueMap> ig_value_map_;
  bool use_ig_value_map_;  ///< Flag to enable IG map

  /**
   * @brief Reset all map buffers for new episode
   * Clears occupancy, distance, and inflation buffers
   */
...
struct MapData2D {
  ...
  std::vector<char> permanent_obstacle_buffer_;  ///< Permanent obstacles that won't be cleared by depth updates
