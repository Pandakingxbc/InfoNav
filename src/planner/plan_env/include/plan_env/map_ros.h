...
#include <plan_env/SemanticPrompts.h>
...
public:
  ...
  void init();
  void resetMapForNewEpisode();  ///< Reset all maps for new episode

  // VLM verification support
  void setCurrentNavTargetObjectId(int object_id) { current_nav_target_object_id_ = object_id; }
  int getCurrentNavTargetObjectId() const { return current_nav_target_object_id_; }
...
private:
  ...
  void semanticPromptsCallback(const plan_env::SemanticPromptsConstPtr& msg);
  void semanticScoresCallback(const plan_env::SemanticScoresConstPtr& msg);
  void episodeResetCallback(const std_msgs::Empty::ConstPtr& msg);  ///< Reset callback for new episodes
  ...
  ros::Publisher occupied_pub_, occupied_inflate_pub_, unknown_pub_, free_pub_, esdf_pub_,
      ...
      value_map_pub_, confidence_map_pub_, ig_map_pub_, hsvm_map_pub_;

  // VLM verification support - publishes when target object is detected
  ros::Publisher object_update_info_pub_;

  // ROS subscribers for sensor data
  ros::Subscriber detected_object_cloud_sub_;
  ros::Subscriber semantic_prompts_sub_, semantic_scores_sub_;
  ...
  // Multi-source semantic value map integration
  vector<string> current_semantic_prompts_;  ///< Current semantic prompts for multi-source value map
  vector<double> current_semantic_scores_;   ///< Current ITM scores for each semantic prompt
  string current_target_object_;             ///< Current navigation target object
  bool use_multi_source_semantic_;           ///< Flag to enable multi-source semantic mode

  // Dual-Value Fusion parameter (shared with exploration_manager)
  double fusion_alpha_;  ///< Weight for semantic value in V_total = alpha * V_sem + (1-alpha) * V_ig

  // VLM verification support - track current navigation target
  int current_nav_target_object_id_;  ///< Current navigation target object ID (-1 if none)
...
inline void MapROS::publishValueMap()
{
  ...
  // Check if multi-source value map is available
  bool use_multi_source = use_multi_source_semantic_ && map_->multi_source_value_map_;
  bool use_ig = map_->use_ig_value_map_ && map_->ig_value_map_;

  for (int x = min_cut(0); x <= max_cut(0); ++x) {
    for (int y = min_cut(1); y <= max_cut(1); ++y) {
      Eigen::Vector2i idx(x, y);

      // Get fused value from HSVM and IG map
      double sem_value = 0.0;
      double ig_value = 0.0;

      if (use_multi_source) {
        sem_value = map_->multi_source_value_map_->getFusedSemanticValue(idx);
      }
      if (use_ig) {
        ig_value = map_->ig_value_map_->getIGValue(idx);
      }

      // Fuse HSVM and IG values: V_total = alpha * (V_sem/4) + (1-alpha) * V_ig
      // HSVM divided by 4 to reduce visual intensity
      if (use_multi_source && use_ig) {
        value = fusion_alpha_ * (sem_value / 4.0) + (1.0 - fusion_alpha_) * ig_value;
      } else if (use_multi_source) {
        value = sem_value / 4.0;
      } else if (use_ig) {
        value = ig_value;
      } else {
        // Fallback to old value map
        value = map_->value_map_->getValue(idx);
      }
...
inline void MapROS::publishIGMap()
{
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x) {
    for (int y = min_cut(1); y <= max_cut(1); ++y) {
      Eigen::Vector2i idx(x, y);
      value = map_->ig_value_map_->getIGValue(idx);

      if (value > 1e-3) {
        // Only show values in free space
        if (map_->md_->occupancy_buffer_inflate_[map_->toAddress(x, y)] == 1)
          continue;
        if (map_->md_->occupancy_buffer_[map_->toAddress(x, y)] < map_->mp_->clamp_min_log_ - 1e-3)
          continue;
  ...
  sensor_msgs::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  ig_map_pub_.publish(cloud_msg);
}

inline void MapROS::publishHSVMMap()
{
  // Use same condition as publishValueMap()
  bool use_multi_source = use_multi_source_semantic_ && map_->multi_source_value_map_;
  if (!use_multi_source)
    return;
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x) {
    for (int y = min_cut(1); y <= max_cut(1); ++y) {
      Eigen::Vector2i idx(x, y);
      value = map_->multi_source_value_map_->getFusedSemanticValue(idx);
      ...
      if (value > 1e-3) {
        ...
        Eigen::Vector2d pos_2d;
        map_->indexToPos(idx, pos_2d);
        // Apply /4 scaling for consistency with algorithm
        value = value / 4.0;
        value = std::min(value, max_value);
        value = std::max(value, min_value);
        pt.x = pos_2d(0);
  ...
  sensor_msgs::PointCloud2 cloud_msg;
  pcl::toROSMsg(cloud, cloud_msg);
  hsvm_map_pub_.publish(cloud_msg);
}
