...
from vlm.detector.dfine import DFineClient
...
# Mode 1: YOLOv7 + GroundingDINO (ApexNav original, better false positive reduction)
# Mode 2: D-FINE + GroundingDINO (current InfoNav, for comparison experiments)
#
# Initialize all detector clients - actual usage depends on mode setting in config
# Port configuration matches start_vlm_servers.sh

# COCO object detectors (mutually exclusive based on mode)
yolov7_detector = YOLOv7Client(port=12184)  # Mode 1: YOLOv7
dfine_detector = DFineClient(port=12185)    # Mode 2: D-FINE

# Shared components (used in both modes)
blip2_itm = BLIP2ITMClient(port=12182)
sam_segmentor = MobileSAMClient(port=12183)
dino_detector = GroundingDINOClient(port=12181)  # Used for non-COCO objects in both modes
...
def get_object(right_label, img, cfg, similar_answer):
    """
    Detect objects in the image using configured detection mode.
    ...
    Args:
        right_label: Target object label(s), separated by '|'
        img: Input RGB image
        cfg: Detector configuration (contains mode and threshold settings)
        similar_answer: List of similar object labels from LLM

    Returns:
        segmented_img: Image with detection visualizations
        score_list: List of detection confidence scores
        object_masks_list: List of segmentation masks
        label_list: List of label indices (0 for target, >0 for similar objects)
    """
    score_list = []
    object_masks_list = []
    ...
    # Get detection mode from config (default: mode 2 for backward compatibility)
    detection_mode = getattr(cfg, 'mode', 2)
    ...
    if coco_label:
        ...
        if detection_mode == 1:
            # Mode 1: YOLOv7 (ApexNav original)
            ...
            print(f"[Mode 1] YOLOv7 detected {len(detections.logits)} objects")
        else:
            # Mode 2: D-FINE (default, current InfoNav)
            conf_thres = getattr(cfg.dfine, 'confidence_threshold_dfine', 0.2)
            detections = dfine_detector.predict(img, conf_thres=conf_thres)
            print(f"[Mode 2] D-FINE detected {len(detections.logits)} objects")
    ...
    if dino_label:
        # GroundingDINO is used for non-COCO objects in both modes
...
def get_object_with_itm(label, img, cfg):
    ...
    segmented_img = img.copy()
    if label in COCO_CLASSES:
        # D-FINE detection (ASCENT approach)
        detections = dfine_detector.predict(img)
